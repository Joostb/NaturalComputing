...
We used the standard \texttt{RandomForestClassifier} from the python module \texttt{sklearn}. We found two variables that we thought to be important: \textit{max\_depth} and \textit{n\_estimators}. These respectively mean the maximum depth of a tree and the number of trees that are trained in the forest. The results are found in figure \ref{fig:mnist_results}.
