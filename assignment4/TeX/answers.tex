...
\begin{itemize}
	...
	\item
	For the radom suspaces, we selected data among each training example. 
\end{itemize}


\section{}
For calculated the importance of variable, we can use the Mean Decrease Impurity. This compute how much each feature decreases the weighted impurity in a tree where the impurity is the measure based on which the optimal condition is chosen.
...
\begin{itemize}
	\item
	n\_estimators which is the number of tree. The more we increase this number, the more the score is stable and the performance seems better, but the code beging slower
	\item
	the max\_depth which is the maximum depth of the tree. If we also increase this number, the performance seems better.
	\item
	the max\_features which is the maximum number of features Random Forest is allowed to try in individual tree.
\end{itemize}
