...
\section{}
In order to implement a random forest, we used the Iris dataset, the sklearn librar and the package RandomForestClassifier. They are 2 parameters that we have try different values : n\_estimators which is the number of tree and the max\_depth which is the maximum depth of the tree. When we increase the number of trees and the maximum of depth, the score seems to increase too (up to a certain threshold). 
